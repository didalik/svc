...
async function WS_USER_URL (data2sign, remoteStr, actorId, USER_PK, USER_SK, SVC_PK) { // {{{2
  let [privateKey, publicKey, host, noget] = await pGET_parms.call(this, remoteStr, 'noget', 'USER', { USER_PK, USER_SK })
  let actorPK = encodeURIComponent(USER_PK)
  let SVC_PK = encodeURIComponent(SVC_PK)
...
export { // {{{1
  WS_USER_URL, generate_keypair, pGET, pGET_parms, signedData, verifyData,
}
